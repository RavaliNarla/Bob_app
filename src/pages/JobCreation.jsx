...
const JobCreation = ({ editRequisitionId, showModal, onClose, editPositionId, onUpdateSuccess, readOnly: readOnlyProp }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (Object.keys(validationErrors).length === 0) {
      try {
        ...
        toast.success(showModal ? t("jobcreation:updated") : t("jobcreation:created"));
        //setFormData(initialState);
        navigate('/job-postings');
      } catch (error) {
    ...
    if (name === "job_relaxation_policy_id") {
      ...
      if (selectedPolicy) {
        ...
        if (allocated > 0 && vacancies <= allocated) {
          newErrors.no_of_vacancies = t("jobcreation:maxVacancies", { allocated });
        }
      }
  ...
  const validateForm = () => {
    const newErrors = {};
    if (!formData.requisition_id) newErrors.requisition_id = t("jobcreation:requisition_id_required");
    if (!formData.position_title.trim()) newErrors.position_title = t("jobcreation:position_title_required");
    if (!formData.dept_id) newErrors.dept_id = t("jobcreation:dept_required");
    // if (!formData.country_id) newErrors.country_id = 'Country is required';
    // if (!formData.state_id) newErrors.state_id = 'State is required';
    // if (!formData.city_id) newErrors.city_id = 'City is required';
    // if (!formData.location_id) newErrors.location_id = 'Location is required';
    //if (!formData.description.trim()) newErrors.description = 'Description is required';
    if (!formData.roles_responsibilities.trim()) newErrors.roles_responsibilities = t("jobcreation:roles_responsibilities_required");
    if (!formData.grade_id) newErrors.grade_id = t("jobcreation:grade_id_required");
    if (!formData.employment_type) newErrors.employment_type = t("jobcreation:employment_type_required");
    if (!formData.eligibility_age_min || isNaN(formData.eligibility_age_min) || Number(formData.eligibility_age_min) <= 0) newErrors.eligibility_age_min = t("jobcreation:min_age_required");
    if (!formData.eligibility_age_max || isNaN(formData.eligibility_age_max) || Number(formData.eligibility_age_max) <= 0) newErrors.eligibility_age_max = t("jobcreation:max_age_required");
    if (!formData.job_relaxation_policy_id) newErrors.job_relaxation_policy_id = t("jobcreation:relaxation_policy_required");
    ...
    if (!formData.mandatory_qualification) newErrors.mandatory_qualification = t("jobcreation:mandatory_qualification_required");
    //if (!formData.preferred_qualification) newErrors.preferred_qualification = 'Preferred Qualification is required';
    if (!formData.mandatory_experience || isNaN(formData.mandatory_experience) || Number(formData.mandatory_experience) <= 0) newErrors.mandatory_experience = t("jobcreation:mandatory_experience_required");
    ...
    if (!formData.documents_required.trim()) newErrors.documents_required = t("jobcreation:documents_required");
    //if (!String(formData.min_credit_score ?? '').trim()) newErrors.min_credit_score = 'Min Credit Score is required';
    if (!formData.no_of_vacancies || isNaN(formData.no_of_vacancies) || Number(formData.no_of_vacancies) <= 0) newErrors.no_of_vacancies = t("jobcreation:positions_required");
    ...
    if (formData.grade_id === '0') {
      ...
      if (!formData.min_salary || isNaN(formData.min_salary) || Number(formData.min_salary) <= 0) {
        newErrors.min_salary = t("jobcreation:min_salary_required");
      } else if (!/^\d+(\.\d{1,2})?$/.test(formData.min_salary)) {
        newErrors.min_salary = t("jobcreation:min_salary_valid");
      }

      // Validate max_salary when grade is 'Others'
      if (!formData.max_salary || isNaN(formData.max_salary) || Number(formData.max_salary) <= 0) {
        newErrors.max_salary = t("jobcreation:max_salary_required");
      ...
      if (formData.min_salary && formData.max_salary &&
        ...
        newErrors.max_salary = t("jobcreation:max_salary_less");
      }
    }
    return newErrors;
  };
    ...
    <Container fluid className="py-3">
        ...
        <Col xs={12} md={10} lg={8}>
          {editPositionId == null && (
            <div className="d-flex justify-content-between align-items-center mb-3 mx-2 buttons_div">
              ...
              <div>
                <h5 className='px-2' style={{ fontFamily: 'Noto Sans', fontWeight: 600, fontSize: '16px', color: '#FF7043', marginBottom: '0px' }}>{t("jobcreation:jobCreation")}</h5>
              </div>
              <div className='d-flex gap-3'>
                ...
                >
                  ...
                  <span> {t("jobcreation:downloadBulkTemplate")}</span>
                </a>
                ...
                >
                  ...
                  <span> {t("jobcreation:uploadBulkJobs")}</span>
                </Button>
              </div>
            </div>
          )}
      ...
      <Modal className='fontss' show={showUploadModal} onHide={() => setShowUploadModal(false)} size="lg" centered>
        <Modal.Header closeButton>
          <Modal.Title className='fonall'>{t("jobcreation:bulkJobCreation")}</Modal.Title>
        </Modal.Header>
        <Modal.Body>
          <div className="upload-section fontss">
            <div className="popform">
              <Form.Group className="mb-2 job-form">
                <Form.Label className="small mb-1 d-flex align-items-center fontss" style={{ gap: '0.4em' }}>
                  {t("jobcreation:selectRequisition")}
                ...
                <Form.Select
                  ...
                  className='fw-300'
                >
                  <option value="">{t("jobcreation:selectRequisition")}</option>
                  {reqs.map((req, index) => (
            ...
            >
              ...
              <div style={{ color: '#757575', fontSize: '0.95rem' }}><span style={{ textDecoration: 'underline', color: '#FF7043', cursor: 'pointer' }}>{t("jobcreation:clickToBrowse")}</span></div>
            </div>
