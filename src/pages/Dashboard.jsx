...
export default function Dashboard() {
  ...
  return (
    <Container fluid className="dashfont py-4">
      <h5 className="mx-4 pb-3" style={{ fontFamily: 'Noto Sans', fontWeight: 600, fontSize: '16px', color: '#FF7043', marginBottom: '0px' }}>{t("dashboard:myDashboard")}</h5>
      <Row>
        <Col md={12} className="">
          {/* Stats Row */}
          {/* <Row className="mb-4">
          ...
          <Row className="mb-4">
            ...
            <Col md={3} className="cardspace">
              <Card className="shadow-sm p-3 bgcolr3">
                <h6>{t("dashboard:pendingApprovals")}</h6>
                <h3>{stats.pending_approvals}</h3>
              </Card>
            </Col>
            <Col md={3} className="cardspace">
              <Card className="shadow-sm p-3 bgcolr7">
                <h6>{t("dashboard:avgTimeToFill")}</h6>
          ...
          <Row>
            <Col md={8} className="cardspace">
              <Card className="shadow-sm p-3 heightcard">
                <h6>{t("dashboard:applicationMetrics")}</h6>
                <Line data={lineData} className="pb-3" />
              </Card>
            </Col>
            <Col md={4} className="cardspace">
              <Card className="shadow-sm p-3 heightcard">
                <h6>{t("dashboard:candidateStatus")}</h6>
          ...
          <div className="mt-4 shortlist shadow-sm mx-2">
            <h5 className="mb-3">{t("dashboard:candidatePipeline")}</h5>
            <div className="pipeline-container">
              {[
                { key: "applied_candidates", label: t("dashboard:applicationsReceived"), color: "#42a5f5" },
                { key: "eligible", label: t("dashboard:eligible"), color: "#ffa726" }, // Orange
                { key: "Shortlisted", label: t("dashboard:shortlisted"), color: "#66bb6a" }, // Green
                { key: "Interviewed", label: t("dashboard:interviewed"), color: "#ab47bc" }, // Purple
                { key: "offers", label: t("dashboard:offered"), color: "#ef5350" }, // Red
                { key: "Joined", label: t("dashboard:joined"), color: "#26c6da" }, // Teal
                { key: "rejected", label: t("dashboard:cancelled"), color: "#8d6e63" } // Brown/Grey
              ].map((item, idx, arr) => (
            ...
            <Col md={12} className="cardspace">
              <Card className="shadow-sm p-3 fonapp">
                <Tabs defaultActiveKey="fulfillment" id="dashboard-tabs" className="mb-3 dashtabs">
                  ...
                  <Tab eventKey="fulfillment" title={t("dashboard:positionFulfillment")}>
                    <Row>
                        ...
                        <Col md={3} key={idx} className="mb-4 d-flex">
                          <Card className="shadow-sm flex-fill text-center p-3 shadowbg">
                            ...
                            <div className="mt-3 text-muted small">
                              {t("dashboard:filled")}: <strong>{p.filled}</strong> | {t("dashboard:inProgress")}: <strong>{p.in_progress}</strong> <br />
                              {t("dashboard:openings")}: <strong>{p.openings}</strong> | {t("dashboard:avgDays")}: <strong>{p.avg_days_open}</strong>
                            </div>
                          </Card>
                        </Col>
                      ))}
                  ...
                  {/* ✅ Open Positions */}
                  <Tab eventKey="openpositions" title={t("dashboard:openPositions")}>
                    <Row>
                        ...
                        <Col md={4} key={idx} className="mb-3">
                          <Card className="h-100 shadow p-2 shadowbg">
                            <Card.Body>
                              ...
                              <p className="mb-1">
                                <span
                                  className={`badge ${pos.position_status === "Active"
                                      ? "bg-success"
                                      : "bg-secondary"
                                    }`}
                                >
                              ...
                              <p className="mb-0 text-muted">
                                {t("dashboard:openings")}: <strong>{pos.total_open_positions}</strong>
                              </p>
                            </Card.Body>
                          </Card>
                        </Col>
                  ...
                  {/* ✅ Upcoming Interviews */}
                  <Tab eventKey="interviews" title={t("dashboard:upcomingInterviews")}>
                      ...
                      {dashboardJson.upcoming_interviews.length > 0 ? (
                        dashboardJson.upcoming_interviews.map((item, idx) => (
                          <ListGroup.Item key={idx}>
                            ...
                            <br />
                            {new Date(item.scheduled_at).toLocaleString()} | <span>{t("dashboard:interviewer")}:</span>{" "}
                            {item.interviewer} | <span>{t("dashboard:candidate")}:</span> {item.candidate_name}
                          </ListGroup.Item>
                        ))
                      ) : (
                        <ListGroup.Item>{t("dashboard:noUpcomingInterviews")}</ListGroup.Item>
                      )}
                    </ListGroup>
                  </Tab>
                </Tabs>
